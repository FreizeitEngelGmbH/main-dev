...
import {
  ...
  partnerGroupActivitiesSeed,
  partnerInquiriesSeed,
  partnerInquiryTemplatesSeed,
  shopPartners,
  shopExperiences,
} from "./data";

/**
 * PARTNER STATIC MOCK BACKEND (static, in-memory, no network).
 *
 * Answers the `/api/partner/*` calls (plus the public shop/home/bundle
 * lookups) made by the Partner screens. It is mounted only around those
 * screens (see `PartnerApp.tsx`), as a nested QueryClientProvider, and is
 ...
 * response is built in memory from ./data and wrapped in a native
 * `Response` so `res.json()` keeps working in the unmodified screens.
 * Edits made in the static build (new groups, inquiry replies, check-ins) live in
 * memory and reset on reload, by design.
 */

// Local, in-memory copy of the sample partner's group activities - mutated
...
let partnerGroupActivities: any[] = [...partnerGroupActivitiesSeed];
let nextGroupActivityId = 8100;

// Local, in-memory copy of the sample partner's inbox (Anfragemanagement) -
...
let partnerInquiries: any[] = partnerInquiriesSeed.map((i) => ({ ...i, replies: [...i.replies] }));
let partnerInquiryTemplates: any[] = [...partnerInquiryTemplatesSeed];
let nextInquiryReplyId = 9700;
let nextTemplateId = 7300;

// Local, in-memory copy of the static bookings, extended with a QR
...
let scannerBookings: any[] = partnerBookings.map((b) => ({
  ...b,
...
export async function apiRequest(
  ...
  if (method === "POST" && replyMatch) {
    ...
    const reply = {
      ...
      inquiryId: id,
      fromType: "partner",
      authorName: partnerProfile.companyName,
  ...
  // no real state machine needed for a read-mostly static dashboard.
...
export const getQueryFn: <T>(options: {
  ...
  async ({ queryKey, signal }) => {
    ...
    if (url === "/api/partner/profile") return partnerProfile as any;
    if (url === "/api/partner/stats") return partnerStats as any;
    if (url === "/api/partner/bookings") return partnerBookings as any;
    if (url === "/api/partner/experiences") return partnerExperiences as any;
    if (url === "/api/partner/settlements") return partnerSettlements as any;
    if (url === "/api/partner/commission-invoices") return partnerCommissionInvoices as any;
    if (url === "/api/partner/resources") return partnerResources as any;
    ...
    if (url === "/api/categories") return categoryList as any;
    ...
    if (url === "/api/experiences?featured=true") return featuredExperiences as any;
    if (url === "/api/partners") return marketplacePartners as any;
    if (url === "/api/cities") return cityList as any;
    if (url === "/api/bundles") return bundleList as any;
    ...
    if (shopPartnerMatch) {
      ...
      return (shopPartners.find((p) => p.id === partnerId) ?? null) as any;
    }
    if (url === "/api/experiences") return shopExperiences as any;

    // Unknown endpoint in static mode - return an empty-ish shape rather than
